...
class KinovaRobot:
    ...
    def moveFrom(self, posicao: vetorCartesiano, orientacao: vetorCartesiano, reference_frame = Base_pb2.CARTESIAN_REFERENCE_FRAME_TOOL):
        ...
        try:
            if reference_frame == Base_pb2.CARTESIAN_REFERENCE_FRAME_BASE:
                feedback = self.base_cyclic.RefreshFeedback()  
                w = Base_pb2.CartesianWaypoint()
                w.reference_frame = reference_frame
                w.pose.x = posicao.x + feedback.base.tool_pose_x        # (meters)
                w.pose.y = posicao.y + feedback.base.tool_pose_y
                w.pose.z = posicao.z + feedback.base.tool_pose_z    # (meters)
                w.pose.theta_x = orientacao.x + feedback.base.tool_pose_theta_x # (degrees)
                w.pose.theta_y = orientacao.y + feedback.base.tool_pose_theta_y # (degrees)
                w.pose.theta_z = orientacao.z + feedback.base.tool_pose_theta_z # (degrees)  
                ...
                while self.is_busy:
                    time.sleep(1)
            elif reference_frame == Base_pb2.CARTESIAN_REFERENCE_FRAME_TOOL:
                w = Base_pb2.CartesianWaypoint()
                w.reference_frame = reference_frame
                w.pose.x = posicao.x      # (meters)
                w.pose.y = posicao.y  # (meters)
                w.pose.z = posicao.z     # (meters)
                w.pose.theta_x = orientacao.x   #(degrees)
                w.pose.theta_y = orientacao.y # (degrees)
                w.pose.theta_z = orientacao.z # (degrees)  
                ...
                while self.is_busy:
                    time.sleep(1)
            elif reference_frame == Base_pb2.CARTESIAN_REFERENCE_FRAME_MIXED:
                feedback = self.base_cyclic.RefreshFeedback()  
                w = Base_pb2.CartesianWaypoint()
                w.reference_frame = reference_frame
                w.pose.x = posicao.x + feedback.base.tool_pose_x        # (meters)
